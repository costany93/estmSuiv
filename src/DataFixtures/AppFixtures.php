...
class AppFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        //Ajout des informations sans club
        for($i = 0; $i<10; $i++){
            $width = mt_rand(300,400);
            $height = mt_rand(300,400);
       ...
       //Ajouter des discussion
       /*
       for($i = 0;$i < mt_rand(1,30);$i++){
            $discussion = new Discussion();
            //initialisation du clubs
            $nclub = $clubTable[mt_rand(0,count($clubTable) - 1)];

            //initialisation de l'étudiant
            $etudiant = $etudiantTable[mt_rand(0,count($etudiantTable) - 1)];

            $discussion->setTitle($faker->paragraph())
                    ->setContent($faker->sentence(200))
                    ->setClub($nclub)
                    ->setEtudiant($etudiant)
            ;
